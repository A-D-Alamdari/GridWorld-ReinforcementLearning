    ...
    def train(self, **kwargs):
        ...
        for episode in range(self.max_episode):
            ...
            while not done:
                ...
                if self.env.grid[next_state_position[0]][next_state_position[1]] == 'G' or \
                        self.env.grid[next_state_position[0]][next_state_position[1]] == 'D':
                    done = True
                TD = reward + self.discount_rate * self.Q[next_state][next_action] - self.Q[old_state][action]
                self.Q[old_state][action] += self.alpha * TD
                state = next_state
                action = next_action
                if TD > max_td:
                    max_td = TD
            ...
            self.max_td_list.append(max_td)
